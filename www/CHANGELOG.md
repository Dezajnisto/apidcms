## 2026-07-31 — v2.2.2 — items_per_page=0 отключает пагинацию для external-страниц

### Исправлено
- v2.2.1 ввёл пагинацию с дефолтом 10 записей на странице — это сломало боковое меню документации (показывало только 10 статей)
- `items_per_page: 0` теперь отключает пагинацию — показывает все записи на одной странице
- Применено к странице `/docs` на apidcms.dezajno.ru
...
### Исправлено
- `BaseController::getUnreadNotificationsCount()` задваивал/затраивал счётчик, когда несколько форм использовали одну таблицу (например, 3 формы → `contacts` → 3× счётчик). Теперь `SELECT DISTINCT source_table` — каждая таблица считается один раз.

### Изменённые файлы
- `core/src/Admin/BaseController.php` — DISTINCT в SQL-запросе
...
### Изменено
- Периоды фильтрации: [1, 7, 14, 30] дней вместо [7, 30, 60, 90]
- Срок хранения по умолчанию: 30 дней (было 90)
- Селект срока хранения в настройках: [7, 14, 30, 60, 90] (было [30, 60, 90, 180, 365])
- Часы пик: метка часа под каждой полоской (0-23), а не только под каждой третьей

### Изменённые файлы
- `core/src/Admin/StatsController.php` — периоды фильтрации
- `core/views/admin/stats/index.html.twig` — переключатель периодов + часы пик
- `core/views/admin/settings/index.html.twig` — селект срока хранения
- `core/init_system_tables.php` — дефолт 30 дней
- `core/src/Front/FrontController.php` — фолбэк 30 дней
